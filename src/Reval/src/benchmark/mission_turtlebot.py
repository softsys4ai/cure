...
if __name__ == "__main__":
    ...
    obs_1 = subprocess.check_call("gnome-terminal -- python moving_obs_1.py '%s'", cwd="service", shell=True)
    # time.sleep(1)
    client.send_goal(goal)
    client.wait_for_result()  
    pose_turtlebot.GetCurrPose()
    ...
    client.send_goal(goal)
    time.sleep(8)
    obs_1 = subprocess.check_call("gnome-terminal -- python moving_obs_2.py '%s'", cwd="service", shell=True)
    client.wait_for_result()      
    pose_turtlebot.GetCurrPose()
    cx = open("log/cx.txt", "r")
    cy = open("log/cy.txt", "r")
