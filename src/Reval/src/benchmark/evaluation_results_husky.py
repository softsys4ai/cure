...
df_eval = pd.DataFrame({
                ...
                "Euclidean_distance":[read_euclidean_distance],
                "RNS":[read_rns],
...
df_config = pd.read_csv('log/config_husky.csv')
df_result = pd.concat([df_config, df_eval], axis=1)
if not os.path.isfile('log/eval_husky.csv'):            
    df_result.to_csv("log/eval_husky.csv", mode='a', index=False, header=True) 
else:
    df_result.to_csv("log/eval_husky.csv", mode='a', index=False, header=False)

df = pd.read_csv('log/eval_husky.csv')
timestamp = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p") 
df.to_csv(f"../../results/Evaluation_results_husky_{timestamp}.csv", mode='a', index=False, header=True) 

df_table = pd.read_csv('log/eval_husky.csv')
print(tabulate(df_table.loc[:, 'RNS':'Mission_success'], headers='keys', tablefmt='grid'))
